...
##
"""
EasyBuild support for installing software using 'mamba', implemented as an easyblock.
...
class Mamba(Binary):
    """Support for installing software using 'mamba'."""

    @staticmethod
    def extra_options(extra_vars=None):
        """Extra easyconfig parameters specific to Mamba easyblock."""
        extra_vars = Binary.extra_options(extra_vars)
        extra_vars.update({
            'channels': [None, "List of conda channels to pass to 'mamba install'", CUSTOM],
            'environment_file': [None, "Conda environment.yml file to use with 'mamba env create'", CUSTOM],
            'remote_environment': [None, "Remote conda environment to use with 'mamba env create'", CUSTOM],
            'requirements': [None, "Requirements specification to pass to 'mamba install'", CUSTOM],
        })
        return extra_vars
    ...
    def extract_step(self):
        ...
        if self.src:
            super(Mamba, self).extract_step()

    def install_step(self):
        """Install software using 'mamba env create' or 'mamba create' & 'mamba install'."""

        # initialize conda environment
        # setuptools is just a choice, but *something* needs to be there
        #cmd = "mamba config --add create_default_packages setuptools"
        ...
        if self.cfg['environment_file'] or self.cfg['remote_environment']:
            ...
            cmd = "%s mamba env create --force %s -p %s" % (self.cfg['preinstallopts'], env_spec, self.installdir)
            run_cmd(cmd, log_all=True, simple=True)
            ...
            cmd = "%s mamba create --force -y -p %s %s" % (self.cfg['preinstallopts'], self.installdir, install_args)
            run_cmd(cmd, log_all=True, simple=True)
