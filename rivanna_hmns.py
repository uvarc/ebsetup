...
class RivannaHMNS(ModuleNamingScheme):
    ...
    def det_short_module_name(self, ec):
        ...
        if ec['name'] == 'R':
            ...
            name = ec['name'].lower()
        return os.path.join(name, self.det_full_version(ec))
    ...
    def det_modpath_extensions(self, ec):
        ...
        if is_compiler or non_system_cuda:
            ...
            excluded_comps = ['GCC', 'NVHPC', 'intel']
            ...
            if ec['name'] in extend_comps:
                ...
                comp_name_ver = [ec['name'].lower(), self.det_full_version(ec)]
        ...
        # special case for Cray toolchains
            ...
            if any(ec.name.startswith(x) for x in CRAY_TOOLCHAIN_NAME_PREFIXES):
                paths.append(os.path.join(TOOLCHAIN, ec.name.lower(), ec.version))
    ...
    def is_short_modname_for(self, short_modname, name):
        ...
        """
        modname_regex = re.compile(r'^%s(/\S+)?$' % re.escape(name.lower()))
        ...
        self.log.debug("Checking whether '%s' is a module name for software with name '%s' via regex %s: %s",
                       short_modname.lower(), name.lower(), modname_regex.pattern, res)
